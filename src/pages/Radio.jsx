...
export default function Radio() {
    useEffect(() => {
        document.title = "Радио | Spot&Choo's";
    }, []);
    ...
    useEffect(() => {
        const fetchNowPlaying = async () => {
            try {
                const res = await fetch('https://radio.spotandchoos.com/api/nowplaying/1?t=' + Date.now());
    ...
    const tickerText = "СЧАСТЛИВЫЕ ЧАСЫ С 12:00 ДО 16:00: ЗАБИРАЙ 20% БАЛЛАМИ В ПРИЛОЖЕНИИ Spot&Choo's /// НОВЫЙ БЕКОН ДЖЕМ БУРГЕР УЖЕ В ОСНОВНОМ МЕНЮ /// 100% МРАМОРНАЯ ГОВЯДИНА /// ТВОЯ ЛЮБИМАЯ МУЗЫКА И ГРЯЗНЫЕ БУРГЕРЫ /// ";
    ...
    return (
        <div className="fixed inset-0 w-full h-full flex items-center justify-center bg-black overflow-hidden pointer-events-auto">
            ...
            <audio id="snc-radio" ref={audioRef} preload="none" playsInline crossOrigin="anonymous" style={{ display: 'none' }}>
                <source src="https://radio.spotandchoos.com/listen/snc_radio/radio.mp3" type="audio/mpeg" />
            </audio>
